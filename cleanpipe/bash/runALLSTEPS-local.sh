...
# usage example:  ./runALL.sh cpt cpt cpt cpt dna_in_water.gro dna_in_water.top 10
# you have to be inside the system folder, where the top and gro are
# the folders will be created where the script is

# these are the arguments:
#procedures that must be done for EM (c means create folders and mdp files, p means preprocess, r means run)
#              ||                 NVT
#              ||                 NPT
#              ||                 PROD 
#name of gro
...
if [ "$#" -ne 8 ]; then
    echo "Wrong number or arguments. Usage: $0 <procedures_EM> <procedures_NVT> <procedures_NPT> <procedures_PROD> <gro_file> <top_file> <production_duration_ns> <groups_to_monitor>"
    exit 1
fi


echo "Procedures to be done (c = create folders and mdp files, p = preprocess, r = run)"

PROCEDURES_EM=$1
echo "    for EM: ${PROCEDURES_EM}"

PROCEDURES_NVT=$2
echo "    for NVT: ${PROCEDURES_NVT}"

PROCEDURES_NPT=$3
echo "    for NPT: ${PROCEDURES_NPT}"

PROCEDURES_PROD=$4
echo "    for PROD: ${PROCEDURES_PROD}"
...
if [[ "${PROCEDURES_PROD}" == *"c"* ]]; then
    if [[ ! -d "4_PROD" ]]; then
        mkdir 4_PROD
        echo "  folder 4_PROD created"
    else
        echo "  folder 4_PROD already exists, skipping creation."
    fi
fi
...
EOT
echo "  4_PROD/${file_prod_mdp} created"
fi
...
if [[ "${PROCEDURES_NVT}" == *"p"* ]]; then
    ...
    if [[ ! -f "2_NVT/${file_nvt_mdp}"  ]]; then
        ...
        cd 2_NVT || exit
        gmx grompp -f $file_nvt_mdp -c "../1_EM/em.gro" -r "../1_EM/em.gro" -p "../${TOP}" -o "nvt.tpr" -maxwarn 1 > grompp.out 2> grompp.err # HERE!!!!!!!!!
        # Check if there were any errors during grompp
        if [ $? -ne 0 ]; then
            echo "  grompp failed"
            cat grompp.err
...
if [[ "${PROCEDURES_NPT}" == *"p"* ]]; then
    ...
    if [[ ! -f "3_NPT/${file_npt_mdp}"  ]]; then
        ...
        cd 3_NPT || exit
        gmx grompp -f $file_npt_mdp -c "../2_NVT/nvt.gro" -r "../2_NVT/nvt.gro" -p "../${TOP}" -o "npt.tpr" -maxwarn 1 > grompp.out 2> grompp.err # HERE!!!!!!!!!
        # Check if there were any errors during grompp
        if [ $? -ne 0 ]; then
            echo "  grompp failed"
            cat grompp.err
...
######################### PROD prep #########################
if [[ "${PROCEDURES_PROD}" == *"p"* ]]; then
    echo "executing grompp for PROD..."
    # Check if the mdp file exist
    if [[ ! -f "4_PROD/${file_prod_mdp}"  ]]; then
        echo "  Error: 4_PROD/${file_prod_mdp} do not exist."
        exit 1
    else
        
        cd 4_PROD || exit
        gmx grompp -f $file_prod_mdp -c "../3_NPT/npt.gro" -p "../${TOP}" -o "prod.tpr" -maxwarn 1 > grompp.out 2> grompp.err # HERE!!!!!!!!!
        # Check if there were any errors during grompp
        if [ $? -ne 0 ]; then
            echo "  grompp failed"
            cat grompp.err
...
######################### PROD run #########################
if [[ "${PROCEDURES_PROD}" == *"r"* ]]; then
    echo "executing mdrun for PROD..."
    # Check if the tpr file exist
    if [[ ! -f "4_PROD/prod.tpr"  ]]; then
        echo "  Error: 4_PROD/prod.tpr do not exist."
        exit 1
    else
...
if [[ "${PROCEDURES_PROD}" == *"r"* ]]; then
    ...
    if [[ ! -f "4_PROD/prod.xtc"  ]]; then
        echo "  Error: 4_PROD/prod.xtc do not exist."
        exit 1
    else
