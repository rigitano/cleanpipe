...
# usage example:  ./runSTEERED.sh a12HW a12HW.gro a12HW.top 1 298 charmm36 pc 2 8

# ATTENTION: PULLING IN THE Z DIRECTION.
# NO ARQUIVO NDX, A MOLECULA QUE MOVE DEVE SE CHAMAR "EMPURRADA", E O ENGASTAMENTO REFERENCIA DEVE SE CHAMAR "ENGASTAMENTO"
# GERALMENTE O ENGASTAMENTO EH A COORDENADA CENTRAL DE UMA MEMBRANA, E A EMPURRADA EH UM PEPTIDEO, DROGA, OU LIGAND.

# ARGUMENTS:
# 1-name that goes on the runSTEERED_<>, and the job name
# 2-gro
# 3-top
...
# Definition of the name of the mdp file
...
pull_group1_name     = LIG    ; all ligand atoms in index.ndx
pull_group2_name     = MEMB   ; I think its all membrand atoms in index.ndx

pull_coord1_type     = umbrella    ; harmonic bias
pull_coord1_geometry = direction ; linear pull
pull_coord1_dim      = N N Y ; pull along z axis only
pull_coord1_groups   = 1 2 ; ligand vs membrane
pull_coord1_start    = yes ; reference COM at t=0
pull_coord1_rate     = 0.001 nm/ps (1 nm/ps, slow pulling)
...
if planned_steps_reached "prod.tpr" "prod.cpt"; then
  ...
  ${GMX} grompp -f ${file_prod_mdp} -c "../${GRO}" -p "../${TOP}" -o "prod.tpr" -maxwarn 1 2>&1 | tee "outanderr.grompp"
