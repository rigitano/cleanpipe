...
# usage example: ./runFEP.sh alaHO alaHO.gro alaHO.top 100 "283 298 313" charmm36 pc molecule_0
...
#MSUB   -n 8                                 # Number of tasks in parallel mode (ntmpi)
...
OMP_NUM_THREADS=2      # number of OpenMP threads (ntomp)
...
#SBATCH --output=t${t}.l${i}.scheduler.out.and.err
...
MDRUN_OPTIONS="-ntomp 16 -ntmpi 1" #this requires  #SBATCH --cpus-per-task=16
...
if [[ $ARCHITECTURE == "slurm" ]]; then
    ...
    sbatch t${t}.l${i}.sh
    echo "job was sent (t: ${t} Lambda: ${i})"
    ...
    #if [ $? -ne 0 ]; then
    #echo "script t${t}.l${i}.sh failed — stopping main script."
    #exit 1
    #fi
